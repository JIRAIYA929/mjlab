...
#把全局地形扫描 terrain_scan 的参考坐标系改到 pelvis
...
#把脚底高度扫描 foot_height_scan 绑定到 left_foot / right_foot 两个 site
  ...
  for sensor in cfg.scene.sensors or ():
    if sensor.name == "foot_height_scan":
      ...
      #使用半径 3 cm、6 点圆环采样脚底局部地形高度
      sensor.pattern = RingPatternCfg.single_ring(radius=0.03, num_samples=6)

#添加脚与地面的接触传感器 feet_ground_contact
...
#如果存在地形生成器，则开启 terrain curriculum
...
#pose reward 的标准差配置（姿态偏离参考姿态多少算“可以接受”。数值越小，约束越紧；数值越大，允许动作幅度越大）
...
#奖励项绑定
  ...
  # Replace the saturating upright bonus with the upstream continuous tilt cost.
